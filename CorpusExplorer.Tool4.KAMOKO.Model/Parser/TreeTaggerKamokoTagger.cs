...
{
  public class TreeTaggerKamokoTagger : AbstractKamokoTagger
  {
    /// <summary>
    ///   The _doc lemma.
    /// </summary>
    ...
    private readonly object _taggerLock = new object();
    ...
    public override void Execute()
    {
      ...
      var layers = new List<AbstractLayerAdapter>
      {
        ...
        LayerAdapterWriteDirect.Create(_docPhrase, _listPhrase, "Phrase"),
      ...
      var corpus = CorpusBuilder.Create(
                                        layers.Select(x => x.ToLayerState()),
                                        _meta,
          ...
          try
          {
            var process = new Process
            {
              StartInfo =
              {
                ...
                Arguments = $"{fileInput.Path} > {fileOutput.Path}",
                CreateNoWindow = true,
        ...
        while (InternQueue.Count > 0)
          Parallel.For(
                       ...
                       i =>
                       {
                         ...
                         var tmp = string.IsNullOrEmpty(text)
                                     ? new string[0]
                                     : text.Split(new[] {TaggerFileSeparator}, StringSplitOptions.RemoveEmptyEntries);
