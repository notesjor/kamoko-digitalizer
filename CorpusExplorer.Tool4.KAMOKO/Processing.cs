...
namespace CorpusExplorer.Tool4.KAMOKO
{
  ...
  public partial class Processing : AbstractForm
  {
    ...
    private string Message
    {
      ...
      set
      {
        if (string.IsNullOrEmpty(value)) value = "Lehnen Sie sich zurück und enspannen Sie sich, während CorpusExplorer für Sie rechnet.";
    ...
    public static void SplashClose(string message, bool itWasrinvoked = false)
    {
      ...
      while (!launched &&
             !itWasrinvoked)
      {
        lock (locker) if (!WaitPlease) launched = true;
      }

      if (_splashForm == null ||
          _splashThread == null) return;
      ...
      if (!string.IsNullOrEmpty(message)) _splashForm.Message = message;
    ...
    {
      if (_splashForm == null) _splashForm = new Processing();
