  ...
  public static class LayerConversionControler
  {
    ...
    private static Course ScrapDocuments(string path)
    {
      ...
      using (var fs = new FileStream(path, FileMode.Open, FileAccess.Read))
      {
        ...
        cases = xml.Deserialize(fs) as List<XmlSpeakerCase>;
      }
    ...
    private static HashSet<char> _isNumber = new HashSet<char> { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', };
      ...
      foreach (var c in cases)
      {
        ...
        while (true)
        {
          if (current + 1 >= c.Text.Length)
            break;
          var next = c.Text.IndexOf("<strong>", current);
          ...
          else if (next == current)
          {
            ...
            next = c.Text.IndexOf("</strong>", current);
                           ...
                           new SpeakerVote
                           {
                             ...
                             Vote =
                               tuple.Value.Item1 == 30
                                 ? new VoteReservation()
                                 : tuple.Value.Item1 == 20 ? (AbstractVote) new VoteAccept() : new VoteDenied()
                           },
                           new SpeakerVote
                           {
                             SpeakerIndex = 2,
                             Vote =
                               tuple.Value.Item2 == 30
                                 ? new VoteReservation()
                                 : tuple.Value.Item2 == 20 ? (AbstractVote) new VoteAccept() : new VoteDenied()
                           },
                           new SpeakerVote
                           {
                             SpeakerIndex = 3,
                             Vote =
                               tuple.Value.Item3 == 30
                                 ? new VoteReservation()
                                 : tuple.Value.Item3 == 20 ? (AbstractVote) new VoteAccept() : new VoteDenied()
                           },
                           new SpeakerVote
                           {
                             SpeakerIndex = 4,
                             Vote =
                               tuple.Value.Item4 == 30
                                 ? new VoteReservation()
                                 : tuple.Value.Item4 == 20 ? (AbstractVote) new VoteAccept() : new VoteDenied()
                           }
                         }
                       });
          }
    ...
    private static string CutOut(string text, int start, int end) => text.Substring(start, end - start).Trim();
